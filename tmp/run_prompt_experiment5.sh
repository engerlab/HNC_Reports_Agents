#!/bin/bash
# Usage: ./run_prompt_experiment5.sh [report_type] [prompt_mode] [case_id]
#   report_type: One of: pathology_reports, consultation_notes, treatment_plan_outcomepred, path_consult_reports, cot_treatment_plan_outcomepred.
#   prompt_mode: Optional prompt suffix (e.g., "combined" or "separated").
#   case_id: Optional; if provided, process only that case (filename without extension). Otherwise, process one random case.
#
# Example (specific case):
#   bash run_prompt_experiment5.sh "path_consult_reports" "combined" "1219243"
#
# Example (random case):
#   bash run_prompt_experiment5.sh "path_consult_reports" "combined"
...
if [ -n "$CASE_ID" ]; then
  ...
  echo "Processing one random case"
fi

#############################
# 4) Run Python Script (Single-Case Experiment)
#############################
LOGFILE="/tmp/summarizer_prompt_experiment5.log"
rm -f "$LOGFILE"
...
python "$PYTHON_SCRIPT" \
  ...
  --single ${CASE_ID:+--case_id "$CASE_ID"} \
  >"$LOGFILE" 2>&1 &
