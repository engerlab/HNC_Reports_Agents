#!/usr/bin/env python3
"""
sampel_selection3.py was used for the manuscript: made the color label of high/medium/low of plots maintain the same across figures.
...
using the new Exp14 directory that contains:
  ...
  - the num_input_tokens in the plots were the max of extr_total_needed and cot_total_needed
    (since this was Experiment mode 2, the max of them will be the max token needed per query.)
...
###############################################################################
# Binning function to categorize number of 'not inferred'
###############################################################################
def categorize_not_inferred(x):
    """
    For uniform color labeling in plots:
      x < 10 => 'low'
      x <= 20 => 'medium'
      otherwise => 'high'
    """
...
def create_scatter_plot(df, category_name, output_dir):
    ...
    # Bin the 'not_inferred_count' so we can color the scatter by low/medium/high
    ...
    out_path_pdf = os.path.join(output_dir, f"{category_name}_scatter.pdf")
    plt.savefig(out_path_pdf, dpi=400, bbox_inches="tight")
...
def create_jointplot(df, category_name, output_dir):
    ...
    # Adjust axis labels, tick labels, title, etc.
    ...
    out_path_pdf = os.path.join(output_dir, f"{category_name}_joint.pdf")
    plt.savefig(out_path_pdf, dpi=600, bbox_inches="tight")
...
def build_category_df(cat_name, cat_ids, df_times, summaries_dir):
    ...
    # ensure these columns exist (others are dropped if not in sub)
    for c in final_cols:
...
def cutoff_selection(df, sample_size, category_type, path_cutoff, cons_cutoff, both_cutoff):
    ...
    final_df.sort_values(by="not_inferred_count", inplace=True)
    logger.info(
        f"[cutoff+sub-bins] {category_type}: requested {sample_size}, got {len(final_df)} out of {len(df_filt)} possible"
    )
    return final_df
...
def main():
    ...
    parser.add_argument("--base_dir", required=True,
                        help="Folder with final_merged_processing_times.csv, precompute_tokens_mode2.csv, "
                             "and text_summaries/mode2_combined_twostep.")
    ...
    parser.add_argument("--both_cutoff", type=int, default=10,
                        help="Cutoff for path_cons category if mode=cutoff.")
    parser.add_argument("--path_cutoff", type=int, default=15,
                        help="Cutoff for path_only if mode=cutoff.")
    parser.add_argument("--cons_cutoff", type=int, default=15,
                        help="Cutoff for consult_only if mode=cutoff.")
...
# Usage example 
# python /Data/Yujing/HNC_OutcomePred/Reports_Agents/utils/sample_selection3.py \
#   --base_dir "/Data/Yujing/HNC_OutcomePred/Reports_Agents_Results/Exp14" \
