...
- Pathology and Consultation Notes are processed via field extraction with regex.
- Treatment Plan Outcome Predictions are processed as narrative outputs.
This script now supports three report types:
    1. pathology_reports
    2. consultation_notes
    3. treatment_plan_outcomepred
Any subfolder other than these is skipped.
"""
...
##############################################################################
# 2. Regex Patterns (Anchored, re.MULTILINE)
##############################################################################
PATTERNS = {
    # Pathology fields
...
def extract_tabular_data(summary: str, report_type: str) -> Dict[str, Any]:
    fields = PATHOLOGY_FIELDS if report_type == "pathology_reports" else CONSULTATION_FIELDS if report_type == "consultation_notes" else []
    if not fields:
...
def validate_extracted_data(data: Dict[str, Any], report_type: str) -> bool:
    if report_type == "treatment_plan_outcomepred":
        return True
    required_fields = PATHOLOGY_FIELDS if report_type == "pathology_reports" else CONSULTATION_FIELDS if report_type == "consultation_notes" else []
...
class ReportSummarizer:
    def __init__(self, prompts_dir: str, model_type: str = "local", temperature: float = 0.3, embedding_model: str = "ollama"):
    ...
    def process_reports(self, input_dir: str, output_dir: str):
        ...
        report_type_map = {
            ...
            "treatment_plan_outcomepred": "treatment_plan_outcomepred"
        }
            ...
            for fname in files:
                ...
                try:
                    ...
                    if not summary:
                        ...
                        continue
                    if rtype in ["pathology_reports", "consultation_notes"]:
                    ...
                    elif rtype == "treatment_plan_outcomepred":
...
def main():
    parser = argparse.ArgumentParser(
        description="Summarize HNC Pathology, Consultation Notes, and Treatment Plan Outcome Predictions. Input directory should contain subfolders: pathology_reports/, consultation_notes/, and treatment_plan_outcomepred/."
    )
    ...
    parser.add_argument("--input_dir", required=True, help="Directory with subfolders: pathology_reports/, consultation_notes/, treatment_plan_outcomepred/.")
    parser.add_argument("--output_dir", required=True, help="Output directory for summaries, embeddings, CSVs.")
    parser.add_argument("--embedding_model", type=str, default="ollama", choices=["ollama","openai","google"], help="Embedding model to use (ollama, openai, google).")
    args = parser.parse_args()
    os.makedirs(args.output_dir, exist_ok=True)
    summarizer = ReportSummarizer(prompts_dir=args.prompts_dir, model_type=args.model_type, temperature=args.temperature, embedding_model=args.embedding_model)
